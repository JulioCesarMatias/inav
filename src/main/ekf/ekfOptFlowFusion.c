...
void SelectFlowFusion(void)
{
    ...
    if (!takeOffDetected && ((terrainState - ekfStates.stateStruct.position.z) < 0.5f))
    {
        ofDataDelayed.flowRadXYcomp.x = 0.0f;
        ofDataDelayed.flowRadXYcomp.y = 0.0f;
        ofDataDelayed.flowRadXY.x = 0.0f;
        ofDataDelayed.flowRadXY.y = 0.0f;
        flowDataValid = true;
    }

    // if have valid flow or range measurements, fuse data into a 1-state EKF to estimate terrain height
    if (((flowDataToFuse && (ekfParam._flowUse == FLOW_USE_TERRAIN)) || rangeDataToFuse) && tiltOK)
    {
    ...
    {
        if (ekfParam._flowUse == FLOW_USE_NAV)
        {
            // Set the flow noise used by the fusion processes
            R_LOS = sq(MAX(ekfParam._flowNoise, 0.05f));
...
void EstimateTerrainOffset(void)
{
    ...
    float velHorizSq = sq(ekfStates.stateStruct.velocity.x) + sq(ekfStates.stateStruct.velocity.y);
    ...
    bool cantFuseFlowData = ((ekfParam._flowUse != FLOW_USE_TERRAIN) || gpsNotAvailable || PV_AidingMode == AID_RELATIVE || velHorizSq < 25.0f || (MAX(ofDataDelayed.flowRadXY.v[0], ofDataDelayed.flowRadXY.v[1]) > ekfParam._maxFlowRate));
    ...
    else
    {
        ...
        float distanceTravelledSq = sq(ekfStates.stateStruct.position.v[0] - prevPosN) + sq(ekfStates.stateStruct.position.v[1] - prevPosE);
        distanceTravelledSq = MIN(distanceTravelledSq, 100.0f);
        prevPosN = ekfStates.stateStruct.position.v[0];
        prevPosE = ekfStates.stateStruct.position.v[1];
        ...
        float Pincrement = (distanceTravelledSq * sq(ekfParam._terrGradMax)) + sq(timeLapsed) * P[5][5];
        Popt += Pincrement;
        ...
        {
            // predict range
            float predRngMeas = MAX((terrainState - ekfStates.stateStruct.position.v[2]), rngOnGnd) / prevTnb.m[2][2];

            // Copy required states to local variable names
            float q0 = ekfStates.stateStruct.quat.q0; // quaternion at optical flow measurement time
            float q1 = ekfStates.stateStruct.quat.q1; // quaternion at optical flow measurement time
            float q2 = ekfStates.stateStruct.quat.q2; // quaternion at optical flow measurement time
            float q3 = ekfStates.stateStruct.quat.q3; // quaternion at optical flow measurement time

            // Set range finder measurement noise variance. TODO make this a function of range and tilt to allow for sensor, alignment and AHRS errors
            float R_RNG = ekfParam._rngNoise;
            ...
            terrainState = MAX(terrainState, ekfStates.stateStruct.position.v[2] + rngOnGnd);

            // Calculate the measurement innovation
            innovRng = predRngMeas - rangeDataDelayed.rng;

            // calculate the innovation consistency test ratio
            auxRngTestRatio = sq(innovRng) / (sq(MAX(0.01f * (float)ekfParam._rngInnovGate, 1.0f)) * varInnovRng);
        ...
        if (!cantFuseFlowData)
        {
            ...
            fpVector2_t losPred;                      // predicted optical flow angular rate measurement
            float q0 = ekfStates.stateStruct.quat.q0; // quaternion at optical flow measurement time
            float q1 = ekfStates.stateStruct.quat.q1; // quaternion at optical flow measurement time
            float q2 = ekfStates.stateStruct.quat.q2; // quaternion at optical flow measurement time
            float q3 = ekfStates.stateStruct.quat.q3; // quaternion at optical flow measurement time
            float K_OPT;
            float H_OPT;
            fpVector2_t auxFlowObsInnovVar;

            // predict range to centre of image
            float flowRngPred = MAX((terrainState - ekfStates.stateStruct.position.z), rngOnGnd) / prevTnb.m[2][2];

            // constrain terrain height to be below the vehicle
            terrainState = MAX(terrainState, ekfStates.stateStruct.position.z + rngOnGnd);

            // calculate relative velocity in sensor frame
            relVelSensor = multiplyMatrixByVector(prevTnb, ekfStates.stateStruct.velocity);
            ...
            auxFlowObsInnov.x = losPred.x - ofDataDelayed.flowRadXYcomp.x;
            auxFlowObsInnov.y = losPred.y - ofDataDelayed.flowRadXYcomp.y;
            ...
            float t4 = q2 * q2;
            float t5 = q3 * q3;
            float t6 = ekfStates.stateStruct.position.z - terrainState;
            float t7 = 1.0f / (t6 * t6);
            float t8 = q0 * q3 * 2.0f;
            ...
            float flow_noise_variance = sq(MAX(ekfParam._flowNoise, 0.05f));

            // Fuse Y axis data

            // Calculate observation partial derivative
            H_OPT = t7 * t9 * (-ekfStates.stateStruct.velocity.z * (q0 * q2 * 2.0 - q1 * q3 * 2.0) + ekfStates.stateStruct.velocity.x * (t2 + t3 - t4 - t5) + ekfStates.stateStruct.velocity.y * (t8 + q1 * q2 * 2.0));
            ...
            // calculate the innovation consistency test ratio
            auxFlowTestRatio.y = sq(auxFlowObsInnov.y) / (sq(MAX(0.01f * (float)ekfParam._flowInnovGate, 1.0f)) * auxFlowObsInnovVar.y);
            ...
            if (auxFlowTestRatio.y < 1.0f)
            {
                ...
                t6 = ekfStates.stateStruct.position.z - terrainState;
                t7 = 1.0f / (t6 * t6);
            ...
            // fuse X axis data
            H_OPT = -t7 * t9 * (ekfStates.stateStruct.velocity.z * (q0 * q1 * 2.0 + q2 * q3 * 2.0) + ekfStates.stateStruct.velocity.y * (t2 - t3 + t4 - t5) - ekfStates.stateStruct.velocity.x * (t8 - q1 * q2 * 2.0));
            ...
            // calculate the innovation consistency test ratio
            auxFlowTestRatio.x = sq(auxFlowObsInnov.x) / (sq(MAX(0.01f * (float)ekfParam._flowInnovGate, 1.0f)) * auxFlowObsInnovVar.x);
...
void FuseOptFlow(void)
{
    ...
    float vn = ekfStates.stateStruct.velocity.x;
    float ve = ekfStates.stateStruct.velocity.y;
    float vd = ekfStates.stateStruct.velocity.z;
    float pd = ekfStates.stateStruct.position.z;
    ...
    { // fuse X axis data first
        // calculate range from ground plane to centre of sensor fov assuming flat earth
        float range = constrainf((heightAboveGndEst / prevTnb.m[2][2]), rngOnGnd, 1000.0f);
        ...
        fpVector3_t posOffsetBody = ofDataDelayed.body_offset;
        if (posOffsetBody.x != 0.0f && posOffsetBody.y != 0.0f && posOffsetBody.z != 0.0f)
        {
            ...
            range -= posOffsetEarth.z / prevTnb.m[2][2];
        }

        // override with user specified height (if given, for rover)
        if (STATE(ROVER) && ofDataDelayed.heightOverride > 0)
        {
        ...
        fpVector3_t mulResult = multiplyMatrixByVector(prevTnb, ekfStates.stateStruct.velocity);
        fpVector3_t remainderResult = {.v = {ofDataDelayed.bodyRadXYZ.y * posOffsetBody.z - ofDataDelayed.bodyRadXYZ.z * posOffsetBody.y,
                                             ofDataDelayed.bodyRadXYZ.z * posOffsetBody.x - ofDataDelayed.bodyRadXYZ.x * posOffsetBody.z,
                                             ofDataDelayed.bodyRadXYZ.x * posOffsetBody.y - ofDataDelayed.bodyRadXYZ.y * posOffsetBody.x}};
        relVelSensor.x = mulResult.x + remainderResult.x;
        relVelSensor.y = mulResult.y + remainderResult.y;
        relVelSensor.z = mulResult.z + remainderResult.z;
        ...
        // calculate the innovation consistency test ratio
        flowTestRatio[obsIndex] = sq(innovOptFlow[obsIndex]) / (sq(MAX(0.01f * (float)ekfParam._flowInnovGate, 1.0f)) * varInnovOptFlow[obsIndex]);

        // Check the innovation for consistency and don't fuse if out of bounds or flow is too fast to be reliable
        if ((flowTestRatio[obsIndex]) < 1.0f && (ofDataDelayed.flowRadXY.x < ekfParam._maxFlowRate) && (ofDataDelayed.flowRadXY.y < ekfParam._maxFlowRate))
        {
            ...
            if (healthyFusion)
            {
                ...
                for (uint8_t j = 0; j <= stateIndexLim; j++)
                {
                    ekfStates.statesArray[j] = ekfStates.statesArray[j] - Kfusion[j] * innovOptFlow[obsIndex];
                }
