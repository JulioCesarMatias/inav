...
void FuseAirspeed(void)
{
    ...
    float vwn;
    float vwe;
    const float R_TAS = sq(constrainf(ekfParam._easNoise, 0.5f, 5.0f));
    Vector3 SH_TAS;
    float SK_TAS;
    Vector24 H_TAS;
    float VtasPred;
    ...
    if (VtasPred > 1.0f)
    {
        ...
        tasTestRatio = sq(innovVtas) / (sq(MAX(0.01f * (float)ekfParam._tasInnovGate, 1.0f)) * varInnovVtas);
        ...
        if (tasHealth || (tasTimeout && posTimeout))
        {
            ...
            {
                ekfStates.statesArray[j] = ekfStates.statesArray[j] - Kfusion[j] * innovVtas;
            }
