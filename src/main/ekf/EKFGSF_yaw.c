...
// Parameters
float EKFGSF_gyroNoise = 1.0e-1;    // yaw rate noise used for covariance prediction (rad/sec)
float EKFGSF_accelNoise = 2.0;      // horizontal accel noise used for covariance prediction (m/sec**2)
float EKFGSF_tiltGain = 0.2;        // gain from tilt error to gyro correction for complementary filter (1/sec)
float EKFGSF_gyroBiasGain = 0.04;   // gain applied to integral of gyro correction for complementary filter (1/sec)
float EKFGSF_accelFiltRatio = 10.0; // ratio  of time constant of AHRS tilt correction to time constant of first order LPF applied to accel data used by ahrs
...
fpVector3_t ahrs_accel; // filtered body frame specific force vector used by AHRS calculation (m/s/s)
...
bool EKFGSF_yaw_correct(const uint8_t mdl_idx, const fpVector2_t vel, const float velObsVar);

// Forces symmetry on the covariance matrix for the selected EKF
void EKFGSF_yaw_forceSymmetry(const uint8_t mdl_idx);
...
float EKFGSF_yaw_gaussianDensity(const uint8_t mdl_idx);

void EKFGSF_yaw_update(const fpVector3_t delAng,
                       const fpVector3_t delVel,
                       ...
                       float TAS)
{
    ...
    const fpVector3_t accel = {.v = {delVel.x / fmaxf(delVelDT, 0.001f),
                                     delVel.y / fmaxf(delVelDT, 0.001f),
                                     delVel.z / fmaxf(delVelDT, 0.001f)}};
    ahrs_accel.x = ahrs_accel.x * (1.0f - filter_coef) + accel.x * filter_coef;
    ahrs_accel.y = ahrs_accel.y * (1.0f - filter_coef) + accel.y * filter_coef;
    ahrs_accel.z = ahrs_accel.z * (1.0f - filter_coef) + accel.z * filter_coef;
    ...
    ahrs_accel_norm = calc_length_pythagorean_3D(ahrs_accel.x, ahrs_accel.y, ahrs_accel.z);
    ...
    fpVector2_t yaw_vector;
    for (uint8_t mdl_idx = 0; mdl_idx < N_MODELS_EKFGSF; mdl_idx++)
    {
        yaw_vector.v[0] += GSF.weights[mdl_idx] * cosf(EKF[mdl_idx].X[2]);
        yaw_vector.v[1] += GSF.weights[mdl_idx] * sinf(EKF[mdl_idx].X[2]);
    }
...
void EKFGSF_yaw_fuseVelData(const fpVector2_t vel, const float velAcc)
{
    ...
    if (run_ekf_gsf)
    {
        ...
        else
        {
            ...
            for (uint8_t mdl_idx = 0; mdl_idx < N_MODELS_EKFGSF; mdl_idx++)
            {
                ...
                if (!EKFGSF_yaw_correct(mdl_idx, vel, velObsVar))
                {
            ...
            if (!state_update_failed)
            {
                ...
                for (uint8_t mdl_idx = 0; mdl_idx < N_MODELS_EKFGSF; mdl_idx++)
                {
                    newWeight[mdl_idx] = EKFGSF_yaw_gaussianDensity(mdl_idx) * GSF.weights[mdl_idx];
...
void EKFGSF_yaw_predictAHRS(const uint8_t mdl_idx)
{
    ...
    const fpVector3_t k = {.v = {AHRS[mdl_idx].R.m[2][0], AHRS[mdl_idx].R.m[2][1], AHRS[mdl_idx].R.m[2][2]}};

    // Calculate angular rate vector in rad/sec averaged across last sample interval
    fpVector3_t ang_rate_delayed_raw = {.v = {delta_angle.x / angle_dt,
                                              delta_angle.y / angle_dt,
                                              delta_angle.z / angle_dt}};
    ...
    fpVector3_t tilt_error_gyro_correction; // (rad/sec)

    if (accel_gain > 0.0f)
    {

        fpVector3_t accel = ahrs_accel;
        ...
        if (true_airspeed >= 0)
        {
            ...
            fpVector3_t centripetal_accel_vec_bf = {.v = {0.0f, ang_rate_delayed_raw.v[2] * true_airspeed, -ang_rate_delayed_raw.v[1] * true_airspeed}};

            // Correct measured accel for centripetal acceleration
            accel.x -= centripetal_accel_vec_bf.x;
            accel.y -= centripetal_accel_vec_bf.y;
            accel.z -= centripetal_accel_vec_bf.z;
        }

        fpVector3_t remainderResult = {.v = {k.y * accel.z - k.z * accel.y,
                                             k.z * accel.x - k.x * accel.z,
                                             k.x * accel.y - k.y * accel.x}};

        tilt_error_gyro_correction.x = remainderResult.x * (accel_gain / ahrs_accel_norm);
        tilt_error_gyro_correction.y = remainderResult.y * (accel_gain / ahrs_accel_norm);
        tilt_error_gyro_correction.z = remainderResult.z * (accel_gain / ahrs_accel_norm);
    }
    ...
    if (spinRate_squared < sq(0.175f))
    {
        ...
        AHRS[mdl_idx].gyro_bias.y -= tilt_error_gyro_correction.y * (EKFGSF_gyroBiasGain * angle_dt);
        AHRS[mdl_idx].gyro_bias.z -= tilt_error_gyro_correction.z * (EKFGSF_gyroBiasGain * angle_dt);

        // sanity check
        if (isnan(AHRS[mdl_idx].gyro_bias.x) || isnan(AHRS[mdl_idx].gyro_bias.y) || isnan(AHRS[mdl_idx].gyro_bias.z))
        {
            vectorZero(&AHRS[mdl_idx].gyro_bias);
        }

        for (uint8_t i = 0; i < 3; i++)
        {
            AHRS[mdl_idx].gyro_bias.v[i] = constrainf(AHRS[mdl_idx].gyro_bias.v[i], -gyro_bias_limit, gyro_bias_limit);
        }
    }
...
void EKFGSF_yaw_alignTilt(void)
{
    ...
    fpVector3_t i_vec_bf = {.v = {1.0f, 0.0f, 0.0f}};
    ...
    {
        R.m[0][col] = north_in_bf.v[col];
        R.m[1][col] = east_in_bf.v[col];
        R.m[2][col] = down_in_bf.v[col];
    }
...
void EKFGSF_yaw_alignYaw(void)
{
    ...
    for (uint8_t mdl_idx = 0; mdl_idx < N_MODELS_EKFGSF; mdl_idx++)
    {
        if (fabsf(AHRS[mdl_idx].R.m[2][0]) < fabsf(AHRS[mdl_idx].R.m[2][1]))
        {
            // get the roll, pitch, yaw estimates from the rotation matrix using a  321 Tait-Bryan rotation sequence
            float roll = atan2f(AHRS[mdl_idx].R.m[2][1], AHRS[mdl_idx].R.m[2][2]);
            float pitch = -asinf(AHRS[mdl_idx].R.m[2][0]);
            float yaw = atan2f(AHRS[mdl_idx].R.m[1][0], AHRS[mdl_idx].R.m[0][0]);

            // set the yaw angle
            yaw = wrap_PI(EKF[mdl_idx].X[2]);

            // update the body to earth frame rotation matrix
            AHRS[mdl_idx].R = matrix_from_euler312(roll, pitch, yaw);
        }
        else
        {
            // Calculate the 312 Tait-Bryan rotation sequence that rotates from earth to body frame
            fpVector3_t euler312 = {.v = {asinf(AHRS[mdl_idx].R.m[2][1]), atan2f(-AHRS[mdl_idx].R.m[2][0], AHRS[mdl_idx].R.m[2][2]), atan2f(AHRS[mdl_idx].R.m[0][1], AHRS[mdl_idx].R.m[1][1])}};
            euler312.v[2] = wrap_PI(EKF[mdl_idx].X[2]); // first rotation (yaw) taken from EKF model state

            // update the body to earth frame rotation matrix
            AHRS[mdl_idx].R = matrix_from_euler312(euler312.v[0], euler312.v[1], euler312.v[2]);
        }
    }
}
...
void EKFGSF_yaw_predict(const uint8_t mdl_idx)
{
    ...
    EKFGSF_yaw_predictAHRS(mdl_idx);
    ...
    if (fabsf(AHRS[mdl_idx].R.m[2][0]) < fabsf(AHRS[mdl_idx].R.m[2][1]))
    {
        ...
        EKF[mdl_idx].X[2] = atan2f(AHRS[mdl_idx].R.m[1][0], AHRS[mdl_idx].R.m[0][0]);
    }
    else
    {
        // use 312 Tait-Bryan rotation to define yaw state
        EKF[mdl_idx].X[2] = atan2f(-AHRS[mdl_idx].R.m[0][1], AHRS[mdl_idx].R.m[1][1]); // first rotation (yaw)
    }

    // calculate delta velocity in a horizontal front-right frame
    fpVector3_t del_vel_NED = multiplyMatrixByVector(AHRS[mdl_idx].R, delta_velocity);
    const float dvx = del_vel_NED.v[0] * cosf(EKF[mdl_idx].X[2]) + del_vel_NED.v[1] * sinf(EKF[mdl_idx].X[2]);
    const float dvy = -del_vel_NED.v[0] * sinf(EKF[mdl_idx].X[2]) + del_vel_NED.v[1] * cosf(EKF[mdl_idx].X[2]);

    // sum delta velocities in earth frame:
    EKF[mdl_idx].X[0] += del_vel_NED.v[0];
    EKF[mdl_idx].X[1] += del_vel_NED.v[1];
    ...
    // force symmetry
    EKFGSF_yaw_forceSymmetry(mdl_idx);
}
...
bool EKFGSF_yaw_correct(const uint8_t mdl_idx, const fpVector2_t vel, const float velObsVar)
{
    ...
    EKF[mdl_idx].innov[0] = EKF[mdl_idx].X[0] - vel.v[0];
    EKF[mdl_idx].innov[1] = EKF[mdl_idx].X[1] - vel.v[1];
    ...
    AHRS[mdl_idx].R.m[0][0] = R_prev[0][0] * cos_yaw - R_prev[1][0] * sin_yaw;
    AHRS[mdl_idx].R.m[0][1] = R_prev[0][1] * cos_yaw - R_prev[1][1] * sin_yaw;
    AHRS[mdl_idx].R.m[0][2] = R_prev[0][2] * cos_yaw - R_prev[1][2] * sin_yaw;
    AHRS[mdl_idx].R.m[1][0] = R_prev[0][0] * sin_yaw + R_prev[1][0] * cos_yaw;
    AHRS[mdl_idx].R.m[1][1] = R_prev[0][1] * sin_yaw + R_prev[1][1] * cos_yaw;
    AHRS[mdl_idx].R.m[1][2] = R_prev[0][2] * sin_yaw + R_prev[1][2] * cos_yaw;
...
void EKFGSF_yaw_resetEKFGSF(void)
{
    ...
    const float yaw_increment = (M_PIf * 2.0f) / (float)N_MODELS_EKFGSF;
    for (uint8_t mdl_idx = 0; mdl_idx < N_MODELS_EKFGSF; mdl_idx++)
    {
...
{
    fpMat3_t ret = R;
    ret.m[0][0] += R.m[0][1] * g.v[2] - R.m[0][2] * g.v[1];
    ret.m[0][1] += R.m[0][2] * g.v[0] - R.m[0][0] * g.v[2];
    ret.m[0][2] += R.m[0][0] * g.v[1] - R.m[0][1] * g.v[0];
    ret.m[1][0] += R.m[1][1] * g.v[2] - R.m[1][2] * g.v[1];
    ret.m[1][1] += R.m[1][2] * g.v[0] - R.m[1][0] * g.v[2];
    ret.m[1][2] += R.m[1][0] * g.v[1] - R.m[1][1] * g.v[0];
    ret.m[2][0] += R.m[2][1] * g.v[2] - R.m[2][2] * g.v[1];
    ret.m[2][1] += R.m[2][2] * g.v[0] - R.m[2][0] * g.v[2];
    ret.m[2][2] += R.m[2][0] * g.v[1] - R.m[2][1] * g.v[0];

    // Renormalise rows
    float rowLengthSq;
    for (uint8_t r = 0; r < 3; r++)
    {
        rowLengthSq = ret.m[r][0] * ret.m[r][0] + ret.m[r][1] * ret.m[r][1] + ret.m[r][2] * ret.m[r][2];
        if (rowLengthSq >= 0)
        {
            ...
            fpVector3_t *row = (fpVector3_t *)&ret.m[0][r];
            row->v[r] *= rowLengthInv;
        }
    }
